...
func (r *accountRepository) BulkUpdate(ctx context.Context, ids []int64, updates service.AccountBulkUpdate) (int64, error) {
	...
	// len(updates.Credentials) > 0 是 Kiro 所需：凭据变更须触发 extra 侧的
	// catalog / generation 清理，缺少它 Kiro 的模型目录会残留到新凭据上。
	if len(updates.Extra) > 0 || len(ollamaGroupIdentityChanges) > 0 || len(opencodeGroupIdentityChanges) > 0 || ollamaProxyIdentityChanged != "" || updates.EnsureCodexFingerprintSeed || len(updates.Credentials) > 0 {
