...
run_api_curl() {
    CURL_ARGS_LOG="$1" HOME="$TEMP_DIR/home" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="${2:-}" \
        ...
        bash -c 'source <(sed "\$d" "$1"); github_api_curl -s "$2"' bash \
        "$ROOT_DIR/deploy/install.sh" "https://api.github.com/repos/doublepi123/sub2api/releases/latest"
}
...
test "$(grep -Fxc 'https://api.github.com/repos/doublepi123/sub2api/releases/latest' "$TEMP_DIR/authenticated")" -eq 1
...
test "$(grep -Fxc 'https://api.github.com/repos/doublepi123/sub2api/releases/latest' "$TEMP_DIR/anonymous")" -eq 1
...
assert_unsafe_invocation_rejected() {
    ...
    if CURL_ARGS_LOG="$TEMP_DIR/$name" PATH="$TEMP_DIR:$PATH" UPDATE_GITHUB_TOKEN="update-secret" \
        bash -c 'source <(sed "\$d" "$1"); shift; github_api_curl "$@"' bash \
...
assert_unsafe_invocation_rejected non-api -s \
    "https://github.com/doublepi123/sub2api/releases/download/v1/asset"
assert_unsafe_invocation_rejected mixed-host -s \
    "https://api.github.com/repos/doublepi123/sub2api/releases/latest" \
    "https://example.com/collect"
assert_unsafe_invocation_rejected multiple-api -s \
    "https://api.github.com/repos/doublepi123/sub2api/releases/latest" \
    "https://api.github.com/repos/doublepi123/sub2api/releases"
